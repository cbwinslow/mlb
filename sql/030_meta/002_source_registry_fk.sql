...
DO $$
BEGIN
   IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_workspace_source_entitlement_source_system') THEN
      ALTER TABLE auth.workspace_source_entitlement
          ADD CONSTRAINT fk_workspace_source_entitlement_source_system
          FOREIGN KEY (source_system_id)
          REFERENCES meta.source_system(source_system_id)
          ON UPDATE RESTRICT
          ON DELETE CASCADE;
